...
class Push:
    ...
    async def send(self):
        ...
        response = await self._jpush._do_request('POST', body, url,
                                                 'application/json',
                                                 version=3)
        return PushResponse(response)

    async def send_validate(self):
        ...
        response = self._jpush._do_request('POST', body, url,
                                           'application/json',
                                           version=3)
        return PushResponse(response)

    async def get_cid(self, count, type=None):
        body = None
        ...
        response = self._jpush._do_request('GET', body, url, 'application/json',
